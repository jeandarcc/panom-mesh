...
function writeDrsConfig(root: string, consumerDir = 'panom-frontend'): void {
  ...
  writeLocalPackage(root, 'panom-hsm-contract', {
    ...
    dependencies: {
      ...
      writeLocalPackage(root, 'panom-arc', {
        name: '@panomapp/arc',
        version: '0.1.3',
        scripts: {
          'pack:check': 'echo pack',
    ...
    JSON.stringify({
      ...
      packages: {
        ...
        '@panomapp/arc': {
          to: ['panom-frontend', 'panom-backend'],
          'only-source': true,
          local: { path: 'panom-arc', build: 'npm run pack:check' },
...
describe('Mesh CI generation', () => {
  ...
  it('uses DRS source packages in the backend workflow when ci.drs is enabled', async () => {
    ...
    expect(output).toContain('Prepare @panomapp/hsm source')
    expect(output).toContain('Prepare @panomapp/subdomain-policy source')
    expect(output).toContain('Prepare @panomapp/arc source')
    expect(output).toContain('working-directory: generated_modules/panom-hsm-contract')
    expect(output).toContain('working-directory: generated_modules/panom-hsm')
    expect(output).toContain('Install DRS dependencies')
    expect(output).toContain(
      'npm install file:./generated_modules/panom-subdomain file:./generated_modules/panom-arc file:./generated_modules/panom-hsm file:./generated_modules/panom-hsm-contract'
    )
    expect(output).toContain('docker build -t')
    expect(output).toContain('-f Dockerfile .')
    expect(output).toContain('Generated by @panomapp/mesh')
    expect(output).toContain('AS builder')
    expect(output).toContain('npm ci --omit=dev')
    expect(output).toContain('COPY --from=builder')
    expect(output).not.toContain('@panomapp/hsm@^1.0.1')
    expect(output).not.toContain('@panomapp/hsm-panom-contract@3.0.0')
  })
  ...
  it('uses DRS source packages in the backend quadlet workflow when ci.drs is enabled', async () => {
    ...
    expect(output).toContain('docker build -t "$BACKEND_IMAGE" -f Dockerfile .')
  ...
  it('generates a mesh-managed backend workflow and runtime bundle when ci.backend.strategy is mesh', async () => {
    ...
    expect(output).toContain('# ' + path.join(root, 'panom-backend', 'Dockerfile'))
    expect(output).toContain('Generated by @panomapp/mesh')
    expect(output).not.toMatch(/FROM node:20-bookworm-slim AS runtime[\s\S]*COPY \.mesh\/runtime-bundle/)
  })

  it('writes backend Dockerfile to the service cwd', async () => {
    const root = makeTempRoot()
    writeDrsConfig(root)
    ...
    const summary = await new CiGenerateCommand(config).generate()
    const dockerfilePath = path.join(root, 'panom-backend', 'Dockerfile')
    expect(summary).toContain('Dockerfile')
    expect(fs.existsSync(dockerfilePath)).toBe(true)
    const dockerfile = fs.readFileSync(dockerfilePath, 'utf8')
    expect(dockerfile).toContain('Generated by @panomapp/mesh')
    expect(dockerfile).toContain('AS builder')
    expect(dockerfile).toContain('npm ci --omit=dev')
    expect(dockerfile).toContain('EXPOSE 8080')
  })

  it('uses plain npm ci in Dockerfile when ci.drs is disabled', async () => {
    const root = makeTempRoot()
    writeDrsConfig(root)
    ...
    const output = await new CiGenerateCommand(config).generate({ print: true })
    const dockerfileSection = output.split('# ' + path.join(root, 'panom-backend', 'Dockerfile'))[1] ?? ''
    expect(dockerfileSection).toContain('RUN npm ci')
    expect(dockerfileSection).toContain('RUN npm ci --omit=dev')
    expect(dockerfileSection).not.toContain('file:./generated_modules/')
  })
  ...
  it('throws when no DRS consumer matches the frontend directory', async () => {
    ...
    await expect(new CiGenerateCommand(config).generate({ print: true })).rejects.toThrow(
      /No DRS consumer matches directory/
    )
  })
})
