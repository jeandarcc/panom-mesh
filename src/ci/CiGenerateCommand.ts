...
`
  ...
  private backendMeshWorkflow(
    ...
    return [
      ...
      '            "$PODMAN_BIN" stop panom >/dev/null 2>&1 || true',
      '            "$PODMAN_BIN" stop panom-worker >/dev/null 2>&1 || true',
      '            "$PODMAN_BIN" rm -f panom >/dev/null 2>&1 || true',
      '            "$PODMAN_BIN" rm -f panom-worker >/dev/null 2>&1 || true',
      '            "$PODMAN_BIN" ps -a --format \'{{.Names}}\' | grep \'^panom-\' | xargs -r "$PODMAN_BIN" rm -f || true',
      '',
