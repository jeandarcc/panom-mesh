...
`
  ...
  private backendMeshWorkflow(
    ...
    return [
      ...
      '        run: |',
          ...
          '            MESH_CLI_REL="./generated_modules/panom-mesh/dist/cli/index.cjs"',
          '',
          '            resolve_node_bin() {',
          ...
          '                return 0',
          '              fi',
          '              for candidate in /usr/bin/podman /usr/local/bin/podman /opt/homebrew/bin/podman; do',
          '                if [ -x "$candidate" ]; then',
          '                  echo "$candidate"',
          '                  return 0',
          '                fi',
          ...
          '              echo "[deploy] ERROR: Node 20+ is required for @panomapp/mesh (found $($NODE_BIN -v))."',
          '              exit 1',
          '            fi',
          '            PODMAN_BIN="$(resolve_podman_bin)" || {',
          '              echo "[deploy] ERROR: podman binary could not be resolved for non-interactive shell."',
          '              exit 1',
          '            }',
          '            SERVICE_PATH="$(dirname "$NODE_BIN"):$(dirname "$PODMAN_BIN"):/usr/local/bin:/usr/bin:/bin"',
      ...
      '              echo "[deploy] node=$(command -v node || true) podman=$(command -v podman || true) resolved_node=$NODE_BIN resolved_podman=$PODMAN_BIN"',
      '              echo "[deploy] service PATH=$SERVICE_PATH"',
      '              systemctl --user cat panom-backend-mesh.service || true',
      '              systemctl --user show panom-backend-mesh.service --property=Environment --property=ExecStart --property=ExecMainStatus --property=ExecMainCode --property=Result --no-pager || true',
      ...
      '              "$PODMAN_BIN" ps -a || true',
      '              ls -la "$APP_DIR/.state" || true',
      '              ls -la "$APP_DIR/.state/logs" || true',
      '              find "$APP_DIR/.state/logs" -maxdepth 1 -type f -name "*.log" -print -exec tail -n 200 {} \\; || true',
      '              exit 1',
      '            fi',
      '',
  ...
  private backendQuadletWorkflow(
    ...
    return [
      ...
      '                sleep 2',
      '              done',
      '              echo "[deploy] ERROR: health check failed at $HEALTH_URL"',
      '              return 1',
      '            }',
      '',
      '            restart_quadlet_unit() {',
      '              local unit_file="$1"',
      '              local unit_name="${unit_file%.container}"',
      '              systemctl --user restart "$unit_name"',
      '            }',
      '',
      "            mapfile -t API_UNITS < <(find \"$QUADLET_DIR\" -maxdepth 1 -type f -name 'panom-api-*.container' -printf '%f\\n' | sort)",
      "            mapfile -t OTHER_UNITS < <(find \"$QUADLET_DIR\" -maxdepth 1 -type f -name 'panom-*.container' ! -name 'panom-mesh-router.container' ! -name 'panom-api-*.container' -printf '%f\\n' | sort)",
      '',
      '            echo "[deploy] Rolling restart of ${#API_UNITS[@]} api unit(s)..."',
      '            for unit in "${API_UNITS[@]}"; do',
      '              echo "[deploy] Restarting ${unit}"',
      '              restart_quadlet_unit "$unit"',
      '              wait_for_mesh_health || exit 1',
      '            done',
      '',
      '            echo "[deploy] Restarting ${#OTHER_UNITS[@]} supporting unit(s)..."',
